...
class PipelineOptions:
    ...
    max_pages: int = 10
    plan_targets: PlanTargets = field(default_factory=PlanTargets)
...
class WebRenewalPipeline:
    ...
    def __init__(
        ...
        *,
        ...
        self.crawler = CrawlerAgent(max_pages=self.options.max_pages)
        ...
        self.plan = PlanProposalAgent(
            accessibility_target=self.options.plan_targets.accessibility,
            seo_target=self.options.plan_targets.seo,
            security_target=self.options.plan_targets.security,
        )
        self.rewrite = RewriteAgent(
            model=self.options.llm.model,
            temperature=self.options.llm.temperature,
            base_url=self.options.llm.base_url,
            api_key=self.options.llm.api_key,
            max_parallel_requests=self.options.llm.max_parallel_requests,
        )
...
def run_pipeline(
    ...
    *,
    ...
    pipeline = WebRenewalPipeline(options=options)
